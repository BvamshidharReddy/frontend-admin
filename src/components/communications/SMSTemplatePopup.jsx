...
const templates = [
  { id: 's1', name: 'Absentee Notification', message: 'Dear Parent, your ward [Student Name] was absent from school today, [Date]. - MedhaShaala' },
  { id: 's2', name: 'PTM Reminder', message: 'Reminder: A Parent-Teacher meeting is scheduled for [Date] at [Time]. Please attend. - MedhaShaala' },
];

export default function SMSTemplatePopup({ open, onOpenChange, classes, students, onSend }) {
  ...
  const handleSend = () => {
    ...
    onSend({
      title: `SMS: ${selectedTemplate.name}`,
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="w-[95vw] sm:max-w-2xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><MessageSquare /> Send SMS from Template</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSend} disabled={!selectedTemplate}><Send className="w-4 h-4 mr-2"/> Send SMS</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={isRequestDialogOpen} onOpenChange={setIsRequestDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Request a New SMS Template</DialogTitle>
            <DialogDescription>Describe the SMS template you need.</DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-3">
            <Label htmlFor="requestDetailsSms">Template Details</Label>
            <Textarea id="requestDetailsSms" value={requestDetails} onChange={e => setRequestDetails(e.target.value)} rows={5} />
          </div>
          <DialogFooter>
